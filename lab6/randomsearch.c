...
static void cleanup_handler(void *arg){
	printf("The thread no %d: number of iterations: %d\n",(*((struct cleanup_t*)arg)).thread_idx,(*((struct cleanup_t*)arg)).iterations);
}


void *search(void *arg){
	struct cleanup_t clean;
     int threadIdx = (*((struct thread_params*)arg)).thread_idx; // indeks watku
     int toFind = (*((struct thread_params*)arg)).target;   // wartosc do odszukania
     int ti = 0;
     int rnd;
	clean.thread_idx = threadIdx;
	
	pthread_cleanup_push(cleanup_handler,&clean);
